    ...
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden font-sans text-slate-800">
      ...
      <div className="md:hidden flex bg-white border-b border-slate-200 no-print flex-shrink-0">
        <button
          ...
          className={`flex-1 py-3 text-sm font-semibold transition-colors ${activeTab === 'form' ? 'text-rose-600 border-b-2 border-rose-600' : 'text-slate-500 hover:bg-slate-50'}`}
        >
          Formulario
        </button>
        <button
          onClick={() => setActiveTab('preview')}
          className={`flex-1 py-3 text-sm font-semibold transition-colors ${activeTab === 'preview' ? 'text-rose-600 border-b-2 border-rose-600' : 'text-slate-500 hover:bg-slate-50'}`}
        >
          Vista Previa
        </button>
      </div>
